...
- var a = bitsToArrays() -> bitsToArrays 객체를 초기화합니다.
- var b = bitsToArrays({max: 10}) -> bitsToArrays 객체를 초기화합니다. 단, 각 비트마스크 최대 크기는 10입니다.
...
반환은 배열의 index값과 해당 index의 내부 배열에 bitmask가 어떻게 설정되어있는지 반환됩니다.
```
return {
  index: 0,
  bitmask: 12
